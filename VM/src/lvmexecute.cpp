 ...
 */
#define VM_PRE(L, ctx) \
    if (!Flow::getInstance().do_pre_op(L, ctx)) \
    { \
        pc++; \
        VM_NEXT(); \
    }
 ...
 */
#define VM_POST(L, ctx) Flow::getInstance().do_post_op(L, ctx)
...
reentry:
    ...
    dispatchContinue:
...
#endif
        {
            VM_CASE(LOP_NOP)
            {
                VM_PRE(L, ExecutionContext((std::uint32_t**)&pc, &cl, (lua_TValue**)&base, &k));
                Instruction insn = *pc++;

                LUAU_ASSERT(insn == 0);
                VM_POST(L, ExecutionContext((std::uint32_t**)&pc, &cl, (lua_TValue**)&base, &k));
                VM_NEXT();
            }
